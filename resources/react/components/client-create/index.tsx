...
export function ClientCreate({ open, onClose, onCreate }: ClientCreateProps) {
  const {
    control,
    handleSubmit,
    setValue,
  ...
  function onSubmit(data: CreateClientForm) {
    console.log('asd')
    Axios.post<CreateClientForm, AxiosResponse<Client>>('/api/clients', data, {
      headers: {
      ...
      .then((result) => {
        if (result.data) {
          context.addSnack('Client added!', 'success')
          onCreate()
          reset()
        }
      })
